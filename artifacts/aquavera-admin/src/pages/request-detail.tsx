    ...
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center h-[60vh] text-center">
          ...
          <p className="text-muted-foreground mb-6">The requested ID {id} does not exist in the system.</p>
    ...
    <AppLayout>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-foreground">Request {request.id}</h1>
              <StatusBadge status={request.status} />
            </div>
            <p className="text-muted-foreground mt-1 text-sm">Submitted on {format(new Date(request.timestamp), 'PPP at p')}</p>
          </div>
        </div>
      </div>
        ...
        <div className="xl:col-span-3 space-y-6">
          <div className="bg-card border border-border rounded-md shadow-sm overflow-hidden">
            <div className="p-5 border-b border-border bg-slate-50/50 flex items-center gap-2">
              ...
              <h2 className="font-semibold text-foreground">Farmer Profile</h2>
            </div>
            <div className="p-6 grid grid-cols-2 gap-y-6 gap-x-8">
              ...
              <div>
                <p className="text-sm text-muted-foreground mb-1">Aadhaar (Masked)</p>
                <p className="font-medium text-foreground font-mono">{request.aadhaar}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground mb-1">Land Record ID</p>
          ...
          <div className="bg-card border border-border rounded-md shadow-sm overflow-hidden">
            <div className="p-5 border-b border-border bg-slate-50/50 flex items-center gap-2">
              ...
              <h2 className="font-semibold text-foreground">Allocation Request</h2>
            </div>
            <div className="p-6 grid grid-cols-2 gap-y-6 gap-x-8">
              <div>
                <p className="text-sm text-muted-foreground mb-1">Declared Crop</p>
                <p className="font-medium text-foreground">{request.cropType}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground mb-1">Duration</p>
                <p className="font-medium text-foreground">{request.durationHours} Hours</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground mb-1">Expected Start</p>
                <p className="font-medium text-foreground">{format(new Date(request.startDate), 'MMM dd, yyyy')}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground mb-1">Calculated Billing</p>
          ...
          <div className="bg-card border border-border rounded-md shadow-sm overflow-hidden">
            <div className="p-5 border-b border-border bg-slate-50/50 flex items-center gap-2">
              ...
              <h2 className="font-semibold text-foreground">AI Analysis & Verification</h2>
            </div>
            <div className="p-6">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-medium text-foreground">Claim vs Reality Confidence</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border border-border rounded-md flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground">Geofence Match</p>
                    <p className="text-xs text-muted-foreground mt-1 mb-2">Location verified against land registry.</p>
                ...
                <div className="p-4 border border-border rounded-md flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground">NDVI Index</p>
                    <p className="text-xs text-muted-foreground mt-1 mb-2">Vegetation density score.</p>
              ...
              {request.confidenceScore < 70 && (
                <div className="mt-4 p-3 bg-red-50 border border-red-100 rounded-md flex items-start gap-3">
                  ...
                  <p className="text-sm text-red-800 font-medium">Anomaly Detected: Crop signature in satellite imagery does not strongly correlate with the declared crop type ({request.cropType}). Manual review required.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="xl:col-span-2 flex flex-col gap-6">
          <div className="bg-card border border-border rounded-md shadow-sm flex flex-col h-full overflow-hidden">
            <div className="flex border-b border-border">
              ...
              >
                <MapPin className="w-4 h-4" /> Satellite Map
              </button>
              <button 
              ...
              >
                <ImageIcon className="w-4 h-4" /> Drone/Field Scan
              </button>
            </div>
              ...
              {activeTab === 'satellite' ? (
                <>
                  ...
                  <div className="absolute bottom-4 left-4 right-4 bg-slate-900/80 backdrop-blur text-white p-3 rounded-md border border-slate-700 text-xs">
                    Last pass: {format(subHours(new Date(request.timestamp), 12), 'PPP p')}
                  </div>
                </>
              ) : (
                <div className="h-full flex items-center justify-center bg-slate-800 relative">
                   ...
                   <div className="absolute top-4 right-4 bg-black/60 text-white text-xs px-2 py-1 rounded backdrop-blur border border-white/20">
                     AI Prediction: {request.confidenceScore < 70 ? 'Unknown' : request.cropType}
                   </div>
                </div>
              )}
            </div>
      ...
      <div className="fixed bottom-0 left-64 right-0 bg-card border-t border-border p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20 flex justify-between items-center px-8">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Action will notify the farmer via SMS.</p>
        </div>
        <div className="flex items-center gap-3">
          <button 
