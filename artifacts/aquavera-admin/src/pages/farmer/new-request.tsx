...
export default function NewRequest() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.verificationData) {
      toast({ title: t("request.verification"), description: t("request.protocol_desc"), variant: "destructive" });
      return;
    }
    setLoading(true);
      ...
      <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-24">
        ...
        <div className="flex items-center gap-4 py-2 border-b border-slate-100 pb-6">
          ...
          <div className="space-y-0.5">
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">{t("request.title")}</h1>
            <p className="text-xs text-slate-500 font-medium tracking-tight">{t("header.smart_portal")}</p>
          </div>
        </div>
          ...
          <div className="space-y-10">
            ...
            <div className="space-y-8">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">{t("request.step1")}</h3>
              </div>

              {/* Crop Category */}
              <div className="space-y-2">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">{t("request.category")}</label>
                <Select 
              ...
              {/* Crop Name */}
              <div className="space-y-2">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">{t("request.crop_name")}</label>
                <Select 
                  value={formData.crop} 
              ...
              {/* Season */}
              <div className="space-y-2">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">{t("request.season")}</label>
                <ToggleGroup 
                  type="single" 
                ...
                >
                  {["kharif", "rabi", "hot"].map((s) => (
                    ...
                    >
                      {s === "hot" ? t("season.hot") : t(`season.${s.toLowerCase()}`)}
                    </ToggleGroupItem>
                  ))}
                </ToggleGroup>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center px-1">
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest">{t("request.land_area")}</label>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">{t("request.verification")}</h3>
              </div>
          ...
          <div className="space-y-10">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">{t("request.estimate")}</h3>
              </div>
                ...
                <CardContent className="p-8 space-y-8">
                  <div className="grid grid-cols-2 gap-y-6 gap-x-10">
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t("request.category")}</label>
                      <p className="text-sm font-bold truncate">{formData.category}</p>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t("request.crop_name")}</label>
                      <p className="text-sm font-bold">{formData.crop}</p>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t("request.season")}</label>
                      <p className="text-sm font-bold capitalize">{formData.season === "hot" ? t("season.hot") : t(`season.${formData.season.toLowerCase()}`)}</p>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t("request.rate_ha")}</label>
                      <p className="text-sm font-bold">₹{billingInfo.ratePerHa}</p>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t("request.area_ha")}</label>
                      <p className="text-sm font-bold">{billingInfo.areaInHa.toFixed(3)} {t("unit.hectare")}</p>
                    </div>
                  </div>
                  ...
                  <div className="pt-8 border-t border-white/10 flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-white/50 font-medium tracking-tight">{t("request.includes_fees")}</p>
                    </div>
              ...
              <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 flex gap-4">
                ...
                <div className="space-y-1">
                  <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest">{t("request.protocol_title")}</h4>
                  <p className="text-[11px] text-slate-500 font-medium leading-relaxed">
                    {t("request.protocol_desc")}
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              {loading ? t("request.processing") : t("request.submit")}
            </Button>
          </div>
